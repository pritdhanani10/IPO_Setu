...
public class BseIpoProvider : IIpoDataProvider
{
    private readonly HttpClient _httpClient;
    private readonly IConfiguration _configuration;
    private readonly ILogger<BseIpoProvider> _logger;

    public string ProviderName => "BSE Official";

    public BseIpoProvider(HttpClient httpClient, IConfiguration configuration, ILogger<BseIpoProvider> logger)
    {
    ...
    {
        var apiKey = _configuration["MarketData:BseApiKey"];
        var endpoint = _configuration["MarketData:BseEndpoint"];

        if (string.IsNullOrWhiteSpace(endpoint))
        {
            _logger.LogInformation("BSE official provider endpoint not configured. Real data sync skipped for this provider.");
        ...
        try
        {
            ...
            {
                _logger.LogWarning("BSE official provider returned status {StatusCode}", response.StatusCode);
                return Enumerable.Empty<Ipo>();
            }
        ...
        {
            _logger.LogError(ex, "Error fetching IPO data from BSE provider.");
            return Enumerable.Empty<Ipo>();
        }
    }
